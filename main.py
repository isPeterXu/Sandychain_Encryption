...
def main():
    ...
    (m_prime, p_prime, h_prime, b, C_prime, c_prime, epk_prime, sigma_prime) = pchba.adapt(sk_delta, m, m_prime, p,
                                                                                           h_prime, b, C, c, epk, sigma,
                                                                                           ID_i, policy_str)
    ...
    if Test_Adapt:
        ...
        while d <= NN:
            ...
            for i in range(trial):
                ...
                ID_i = None
                start = time.time()
                (m_prime, p_prime, h_prime, b, C_prime, c_prime, epk_prime, sigma_prime) = pchba.adapt(sk_delta, m,
                                                                                                       m_prime, p,
                                                                                                       h_prime, b, C, c,
                                                                                                       epk, sigma, ID_i,
                                                                                                       policy_str)
    ...
    if (pchba.judge(m, p, h_prime, b, C, c, epk, sigma, m_prime, p_prime, C_prime, c_prime, epk_prime,
                    sigma_prime) == 0):
        print("Successful verification.")
    else:
        print("Verification failed.")
    ...
    if Test_Judge:
        ...
        while d <= NN:
            ...
            m_prime = None
            ID_i = None
            (m_prime, p_prime, h_prime, b, C_prime, c_prime, epk_prime, sigma_prime) = pchba.adapt(sk_delta, m, m_prime,
                                                                                                   p, h_prime, b, C, c,
                                                                                                   epk, sigma, ID_i,
                                                                                                   policy_str)
            ...
            for i in range(trial):
                start = time.time()
                pchba.judge(m, p, h_prime, b, C, c, epk, sigma, m_prime, p_prime, C_prime, c_prime, epk_prime,
                            sigma_prime)
                end = time.time()
                Temp = end - start
                T += Temp
