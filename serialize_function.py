...
def setup(k, ii, jj):
    ...
    # serialize
    sk_bytes = group.serialize(sk)
    pk_bytes = group.serialize(pk)
    # msk_bytes = {'a0': group.serialize(a0), 'a1': group.serialize(a1), 'b0': group.serialize(b0),
    #              'b1': group.serialize(b1), 'alpha': group.serialize(alpha), 'beta': group.serialize(beta),
    #              'd0': group.serialize(d0), 'd1': group.serialize(d1), 'd2': group.serialize(d2),
    #              'g_d1': group.serialize(g_d1), 'g_d2': group.serialize(g_d2), 'g_d3': group.serialize(g_d3),
    #              'Z': Z_list}
    msk_string = json.dumps(msk, cls=MyEncoder, indent=4)
    mpk_string = json.dumps(mpk, cls=MyEncoder, indent=4)
    ID_i_bytes = group.serialize(ID_i)
    ID_j_bytes = group.serialize(ID_j)

    # deserialize
    # msk_dictstr = json.loads(msk_string)
    # msk_bytes = {
    #     key:
    #     [group.deserialize(x.encode('utf-8')) for x in val] if isinstance(val, list)
    #     else group.deserialize(val.encode())
    #     for key, val in msk_dictstr.items()}

    return sk_bytes, pk_bytes, msk_string, mpk_string, ID_i_bytes, ID_j_bytes
...
def keygen(sk, msk, mpk, attr_list, assump_size, ii, ID_i):
    ...
    msk_dictstr = json.loads(msk)
    msk = {
        key:
            [group.deserialize(x.encode('utf-8')) for x in val] if isinstance(val, list)
            else group.deserialize(val.encode())
        for key, val in msk_dictstr.items()}
    mpk_dictstr = json.loads(mpk)
    mpk = {
        key:
            [group.deserialize(x.encode('utf-8')) for x in val] if isinstance(val, list)
            else group.deserialize(val.encode())
        for key, val in mpk_dictstr.items()}
    attr_list = json.loads(attr_list)
    ID_i = group.deserialize(ID_i)
    ...
    # serialize
    sk_delta_str = json.dumps(sk_delta, cls=MyEncoder, indent=4)

    return sk_delta_str


# deserialize sk_delta
def convert(data):
    data_new = {
        key:
            [group.deserialize(x.encode('utf-8')) for x in val] if isinstance(val, list)
            else group.deserialize(val.encode()) if isinstance(val, str)
            else convert(val)
        for key, val in data.items()}
    return data_new
    # if isinstance(data, str):
    #     return group.deserialize(data.encode())
    # if isinstance(data, dict):
    #     return dict(map(convert, data.values()))
...
def hash(policy_str, msk, mpk, pk, assump_size, ID_j, verbose=False):
    ...
    p_bytes = group.serialize(p)
    h_prime_bytes = group.serialize(h_prime)
    b_bytes = group.serialize(b)
    c_bytes = group.serialize(c)
    epk_bytes = group.serialize(epk)
    sigma_bytes = group.serialize(sigma)
    _C_string = json.dumps(_C, cls=MyEncoder, indent=4)

    return m_bytes, p_bytes, h_prime_bytes, b_bytes, _C_string, c_bytes, epk_bytes, sigma_bytes


def verify(mpk, m, p, h_prime, b, C, c, epk, sigma):
    # deserialize
    m = group.deserialize(m)
    p = group.deserialize(p)
    h_prime = group.deserialize(h_prime)
    b = group.deserialize(b)
    c = group.deserialize(c)
    epk = group.deserialize(epk)
    sigma = group.deserialize(sigma)
    mpk_dictstr = json.loads(mpk)
    mpk = {
        key:
            [group.deserialize(x.encode('utf-8')) for x in val] if isinstance(val, list)
...
def adapt(mpk, msk, sk_delta, m, h_prime, b, C, policy_str, assump_size, ID_j, pk, sk, verbose=False):
    ...
    mpk = {
        ...
        for key, val in mpk_dictstr.items()}
    sk_delta_dictstr = json.loads(sk_delta)
    sk_delta = {
        key:
            val if isinstance(val, list)  # attr_list是str类型
            else group.deserialize(val.encode()) if isinstance(val, str)  # x为element类型
            else convert(val)
        for key, val in sk_delta_dictstr.items()}
